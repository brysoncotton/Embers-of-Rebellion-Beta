...
---@class BuildStructuresGroundMission
BuildStructuresGroundMission = class()

function BuildStructuresGroundMission:new(gc, player)
...
function BuildStructuresGroundMission:update()
	--Logger:trace("entering BuildStructuresGroundMission:update")
...
function BuildStructuresGroundMission:Begin(reward_group, week_start)
	--Logger:trace("entering BuildStructuresGroundMission:Begin")
	if self.Active == true then
		return
	end
	...
	local tag = "BUILDSTRUCTURESGROUND"
	crossplot:publish("MISSION_STARTED", tag)

	Story_Event("BUILDSTRUCTURESGROUND_ASSIGN")
end

function BuildStructuresGroundMission:UpdateDisplay()
	--Logger:trace("entering BuildStructuresGroundMission:UpdateDisplay")
	local plot = Get_Story_Plot("Conquests\\Events\\MissionRepository.xml")
	local event = plot.Get_Event("BuildStructuresGround_01")
	event.Set_Dialog(self.Dialog)
	event.Clear_Dialog_Text()

	event.Add_Dialog_Text("TEXT_INTERVENTION_BUILDSTRUCTURESGROUND_OBJECTIVE")
...
function BuildStructuresGroundMission:Fulfil()
	--Logger:trace("entering BuildStructuresGroundMission:Fulfil")
	...
	local plot = Get_Story_Plot("Conquests\\Events\\MissionRepository.xml")
	local event = plot.Get_Event("BuildStructuresGround_02")
	event.Set_Dialog(self.Dialog)
	event.Clear_Dialog_Text()
	...
	Story_Event("BUILDSTRUCTURESGROUND_COMPLETE")
end

function BuildStructuresGroundMission:Failed()
	--Logger:trace("entering BuildStructuresGroundMission:Failed")

	local plot = Get_Story_Plot("Conquests\\Events\\MissionRepository.xml")
	local event = plot.Get_Event("BuildStructuresGround_04")
	event.Set_Dialog(self.Dialog)
	event.Clear_Dialog_Text()
	...
	if self.RewardGroupTable.GroupSupport == "LEGITIMACY" then
		crossplot:publish("INCREASE_LEGITIMACY", self.RewardGroupTable.DialogName, (0 - self.RewardGroupTable.SupportArg))
	elseif self.RewardGroupTable.GroupSupport then
	...
	Story_Event("BUILDSTRUCTURESGROUND_FAILED")
end

function BuildStructuresGroundMission:Reset()
	--Logger:trace("entering BuildStructuresGroundMission:Reset")
	self.Active = false
	self.Dialog = nil
	self.TimerStart = nil
	self.TimeActive = nil
...
function BuildStructuresGroundMission:on_construction_finished(planet, game_object_type_name)
	--Logger:trace("entering BuildStructuresGroundMission:on_construction_finished")
...
return BuildStructuresGroundMission
