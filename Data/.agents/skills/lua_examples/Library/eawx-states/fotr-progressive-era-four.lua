    ...
    on_enter = function(self, state_context)
        ...
        if self.entry_time <= 5 then
            if Find_Player("local") == Find_Player("Empire") then
                ...
                Story_Event("CLONE_WARS_20BBY_REPUBLIC_START")
            elseif Find_Player("local") == Find_Player("Rebel") then
                ...
                Story_Event("CLONE_WARS_20BBY_CONFEDERACY_START")
            elseif Find_Player("local") == Find_Player("Hutt_Cartels") then
                StoryUtil.Multimedia("TEXT_STORY_INTRO_PROGRESSIVE_HUTT_CARTELS_JABBA_ERA_4", 15, nil, "Jabba_Loop", 0)
                Story_Event("CLONE_WARS_20BBY_HUTT_CARTELS_START")
            end
